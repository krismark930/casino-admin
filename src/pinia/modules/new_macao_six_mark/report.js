...
import { NEW_MACAO_KAGUAN_REPORT } from "@/api";
import { NEW_MACAO_ALL_REPORT } from "@/api";
import { NEW_MACAO_KAZONG_REPORT } from "@/api";
import { NEW_MACAO_KADAI_REPORT } from "@/api";
import { NEW_MACAO_KAUSER_REPORT } from "@/api";
import { NEW_MACAO_TOTAL_BILL } from "@/api";
import { NEW_MACAO_SUB_BILL } from "@/api";

export const macaoReportStore = defineStore('new_macao_report', {
    state: () => ({
        success: false,
        kaguanReportList: [],
        sumKaguanReport: [],
    ...
    actions: {
        ...
        async dispatchAllReport(data) {
            try {
                ...
                let response = await request({ url: NEW_MACAO_ALL_REPORT, method: 'POST', data })
        ...
        async dispatchKaguanReport(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: NEW_MACAO_KAGUAN_REPORT, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchKazongReport(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: NEW_MACAO_KAZONG_REPORT, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchKadaiReport(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: NEW_MACAO_KADAI_REPORT, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchKauserReport(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: NEW_MACAO_KAUSER_REPORT, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchTotalBill(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: NEW_MACAO_TOTAL_BILL, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchMacaoSubBill(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: NEW_MACAO_SUB_BILL, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
