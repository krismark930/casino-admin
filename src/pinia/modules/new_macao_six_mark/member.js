...
import { GET_NEW_MACAO_KAMEM_ALL } from "@/api";
import { UPDATE_NEW_MACAO_MEMBER_STATUS } from "@/api";
import { GET_NEW_MACAO_KAGUAN_ALL } from "@/api";
import { NEW_MACAO_KAGUAN_ALL } from "@/api";
import { UPDATE_NEW_MACAO_KAGUAN_STATUS } from "@/api";
import { GET_NEW_MACAO_KAMEM_SUPERIOR } from "@/api";
import { SAVE_NEW_MACAO_KAMEM } from "@/api";
import { GET_NEW_MACAO_KADAN_SUPERIOR } from "@/api";
import { SAVE_NEW_MACAO_KADAN } from "@/api";
import { GET_NEW_MACAO_KAZONG_SUPERIOR } from "@/api";
import { SAVE_NEW_MACAO_KAZONG } from "@/api";
import { GET_NEW_MACAO_KAGUAN_SUPERIOR } from "@/api";
import { SAVE_NEW_MACAO_KAGUAN } from "@/api";

export const memberStore = defineStore('new_macao_member', {
    state: () => ({
        success: false,
        memberList: [],
        kaguanMemberList: [],
    ...
    actions: {
        ...
        async dispatchGetKamemAll(data) {
            try {
                ...
                let response = await request({ url: GET_NEW_MACAO_KAMEM_ALL, method: 'POST', data })
        ...
        async dispatchKamemSuperior(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: GET_NEW_MACAO_KAMEM_SUPERIOR, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchGetKaguanAll(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: GET_NEW_MACAO_KAGUAN_ALL, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchUpdateMemberStatus(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: UPDATE_NEW_MACAO_MEMBER_STATUS, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
        ...
        async dispatchKaguanByGrade(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: NEW_MACAO_KAGUAN_ALL, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                    this.setMemberList(response.data);
                    this.setTotalCount(response.total_count);
        ...
        async dispatchUpdateKaguanStatus(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: UPDATE_NEW_MACAO_KAGUAN_STATUS, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
        ...
        async dispatchAddKamem(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: SAVE_NEW_MACAO_KAMEM, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
        ...
        async dispatchKadanSuperior(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: GET_NEW_MACAO_KADAN_SUPERIOR, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchAddKadan(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: SAVE_NEW_MACAO_KADAN, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
        ...
        async dispatchKazongSuperior(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: GET_NEW_MACAO_KAZONG_SUPERIOR, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchAddKazong(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: SAVE_NEW_MACAO_KAZONG, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
        ...
        async dispatchKaguanSuperior(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: GET_NEW_MACAO_KAGUAN_SUPERIOR, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchAddKaguan(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: SAVE_NEW_MACAO_KAGUAN, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
