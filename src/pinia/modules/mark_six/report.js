...
import { KAGUAN_REPORT } from "@/api";
import { ALL_REPORT } from "@/api";
import { KAZONG_REPORT } from "@/api";
import { KADAI_REPORT } from "@/api";
import { KAUSER_REPORT } from "@/api";
import { TOTAL_BILL } from "@/api";
import { SUB_BILL } from "@/api";
    ...
    actions: {
        ...
        async dispatchAllReport(data) {
            try {
                ...
                let response = await request({ url: ALL_REPORT, method: 'POST', data })
        ...
        async dispatchKaguanReport(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: KAGUAN_REPORT, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchKazongReport(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: KAZONG_REPORT, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchKadaiReport(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: KADAI_REPORT, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchKauserReport(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: KAUSER_REPORT, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchTotalBill(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: TOTAL_BILL, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchSubBill(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: SUB_BILL, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
