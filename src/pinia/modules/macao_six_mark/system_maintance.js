...
import { GET_MACAO_WEBSITE_SETTING } from "@/api";
import { GET_MACAO_ODD_DIFF_SETTING } from "@/api";
import { UPDATE_MACAO_WEBSITE_SETTING } from "@/api";
import { UPDATE_MACAO_ODD_DIFF_SETTING } from "@/api";
import { GET_MACAO_AUTO_PRECIPITATION } from "@/api";
import { UPDATE_MACAO_AUTO_PRECIPITATION } from "@/api";
import { GET_MACAO_SINGLE_QUOTA } from "@/api";
import { UPDATE_MACAO_SINGLE_QUOTA } from "@/api";
import { GET_MACAO_WATER_SETTING } from "@/api";
import { UPDATE_MACAO_WATER_SETTING } from "@/api";

export const systemMaintanceStore = defineStore('macao_system_maintance', {
    state: () => ({
        success: false,
    ...
    actions: {
        ...
        async dispatchWebsiteSettingItem(data) {
            try {
                ...
                let response = await request({ url: GET_MACAO_WEBSITE_SETTING, method: 'POST', data })
        ...
        async dispatchUpdateWebsiteSetting(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: UPDATE_MACAO_WEBSITE_SETTING, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
        ...
        async dispatchOddDiffSetting(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: GET_MACAO_ODD_DIFF_SETTING, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchUpdateOddDiffSetting(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: UPDATE_MACAO_ODD_DIFF_SETTING, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
        ...
        async dispatchAutoPrecipitation(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: GET_MACAO_AUTO_PRECIPITATION, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchUpdateAutoPrecipitation(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: UPDATE_MACAO_AUTO_PRECIPITATION, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
        ...
        async dispatchSingleQuota(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: GET_MACAO_SINGLE_QUOTA, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchUpdateSingleQuota(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: UPDATE_MACAO_SINGLE_QUOTA, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
        ...
        async dispatchWaterSetting(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: GET_MACAO_WATER_SETTING, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
        ...
        async dispatchUpdateWaterSetting(data) {
            try {
                this.setSuccess(false);
                let response = await request({ url: UPDATE_MACAO_WATER_SETTING, method: 'POST', data })
                if (response.status === 200) {
                    this.setSuccess(true);
                }
            } catch (e) {
