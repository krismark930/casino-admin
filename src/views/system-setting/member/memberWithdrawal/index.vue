      ...
      <el-table-column fixed="right" label="操作" width="100">
        <template #default="scope">
          ...
          <div style="display: flex;">
            <el-button type="primary" link @click="cancelAccessData(scope.row.ID)"
              v-if="scope.row.Cancel == 0 && scope.row.Type == 'T' && !scope.row.Bank_Account.includes('会员余额')">恢复</el-button>
...
export default {
  setup() {
    ...
    const {dispatchCancelAccessData} = accessStore();
    return {
      dispatchAccessData,
      dispatchDeleteAccessData,
      dispatchCancelAccessData
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    async cancelAccessData(ID) {
      var sResult;
      if (confirm("是否确定恢复金额?")) {
        sResult = prompt("请输入拒绝理由：", "");
      }
      this.loading = true;
      await this.dispatchCancelAccessData({ ID, sResult });
      await this.dispatchAccessData(this.formData);
      this.loading = false
    },
