<template>
  <div style="border: 1px solid #eee; padding: 0.75rem; margin-top: 0.75rem; text-align: left;">
    <h3>{{ $t('menu.chessconveresionRecord') }}</h3>
    <el-form :inline="true" :model="formData">
      <el-form-item label="关键字查找">
        <el-input clearable v-model="formData.user_name" placeholder=""></el-input>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="getKYLogsByFilter">确认</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="KYLogList" v-loading="loading" style="width: 100%;" border header-align="center" stripe>
      <el-table-column type="index" label="编号" />
      <el-table-column property="Username" label="帐号" />
      <el-table-column property="KY_User" label="KY真人帐号" />
...
export default {
  setup() {
    const { dispatchKYLogsData } = gameLogStore();
    return {
      dispatchKYLogsData
    }
  },
  ...
  computed: {
    ...
    KYLogList: function() {
      const {getKYLogList} = gameLogStore();
      return getKYLogList;
    }
  },
  methods: {
    async getKYLogsByFilter() {
      this.loading = true;
      await this.dispatchKYLogsData(this.formData)
      this.loading = false;
    },
    async onPageChange() {
      this.loading = true;
