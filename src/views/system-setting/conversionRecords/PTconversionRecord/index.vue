<template>
  <div style="border: 1px solid #eee; padding: 0.75rem; margin-top: 0.75rem; text-align: center;">
    <h3>{{ $t('menu.PTconveresionRecord') }}</h3>
    <el-form :inline="true" :model="formData">
      <el-form-item label="关键字查找">
        <el-input clearable v-model="formData.user_name" placeholder=""></el-input>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="getPTLogsByFilter">确认</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="PTLogList" v-loading="loading" style="width: 100%;" border header-align="center" stripe>
      <el-table-column type="index" label="编号" />
      <el-table-column property="Username" label="帐号" />
      <el-table-column property="PT_User" label="PT真人帐号" />
...
export default {
  setup() {
    const { dispatchPTLogsData } = gameLogStore();
    return {
      dispatchPTLogsData
    }
  },
  ...
  computed: {
    ...
    PTLogList: function() {
      const {getPTLogList} = gameLogStore();
      return getPTLogList;
    }
  },
  methods: {
    async getPTLogsByFilter() {
      this.loading = true;
      await this.dispatchPTLogsData(this.formData)
      this.loading = false;
    },
    async onPageChange() {
      this.loading = true;
