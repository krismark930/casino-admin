<template>
  <div style="border: 1px solid #eee; padding: 0.75rem; margin-top: 0.75rem; text-align: left;">
    ...
    <el-form :inline="true" :model="formData">
      ...
      <el-form-item>
        <el-button type="primary" @click="getMGLogsByFilter">确认</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="MGLogList" v-loading="loading" style="width: 100%;" border header-align="center" stripe>
      <el-table-column type="index" label="编号" />
      <el-table-column property="Username" label="帐号" />
      <el-table-column property="MG_User" label="MG真人帐号" />
...
export default {
  setup() {
    const { dispatchMGLogsData } = gameLogStore();
    return {
      dispatchMGLogsData
    }
  },
  ...
  computed: {
    ...
    MGLogList: function() {
      const {getMGLogList} = gameLogStore();
      return getMGLogList;
    }
  },
  methods: {
    async getMGLogsByFilter() {
      this.loading = true;
      await this.dispatchMGLogsData(this.formData)
      this.loading = false;
    },
    async onPageChange() {
      this.loading = true;
