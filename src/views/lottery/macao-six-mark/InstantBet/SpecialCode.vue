    ...
    <el-col>
      <el-form inline="true">
        <el-form-item :label="$t('lottery.chooseNumberOfPeriods')">
          <el-select placeholder="第2023021期" v-model="selectedPeriod" style="width: 200px">
        ...
        <tbody>
          <tr v-for="n in rowNum2" :key="n" v-if="assistantData.length > 0">
            <template v-for="m in blockNum" :key="m">
              ...
              <template v-else>
                <template v-if="n + (m - 1) * rowNum2 == recordNum2">
                  <td :colspan="colNum">{{ $t('lottery.bettingTotal') }}: {{ assistantAmount }}</td>
                </template>
...
const { dispatchGetSpecialCode } = instantBetStore();

// const props = defineProps({
//   isCurrentTab: String,
// });
// const { isCurrentTab } = toRefs(props);
...
const assistantAmount = computed(() => {
  const { getAssistantAmount } = storeToRefs(instantBetStore());
  return getAssistantAmount.value;
})
...
watch(selectedPeriod, async () => {
  ...
  await dispatchGetSpecialCode({ period: selectedPeriod.value, class1: "特码" });
  loading.close();
}, { deep: true })
const successResult = () => {
  if (success.value) {
