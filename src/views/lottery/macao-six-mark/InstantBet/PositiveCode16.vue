<template>
  <el-row justify="space-between">
    ...
    <el-col>
      <div class="flex">
        <el-button size="small" @click="getDataByClass2('正码1')">
          <el-icon><Plus /></el-icon>
          正码1
        </el-button>
        <el-button size="small" @click="getDataByClass2('正码2')">
          <el-icon><Plus /></el-icon>
          正码2
        </el-button>
        <el-button size="small" @click="getDataByClass2('正码3')">
          <el-icon><Plus /></el-icon>
          正码3
        </el-button>
        <el-button size="small" @click="getDataByClass2('正码4')">
          <el-icon><Plus /></el-icon>
          正码4
        </el-button>
        <el-button size="small" @click="getDataByClass2('正码5')">
          <el-icon><Plus /></el-icon>
          正码5
        </el-button>
        <el-button size="small" @click="getDataByClass2('正码6')">
          <el-icon><Plus /></el-icon>
          正码6
        </el-button>
      </div>
...
import { instantBetStore } from "@/pinia/modules/macao_six_mark/instant_bet.js";
import { storeToRefs } from "pinia";

const { dispatchGetPeriod } = instantBetStore();
const { dispatchGetPositiveCode16 } = instantBetStore();
...
const getDataByClass2 = async (data) => {
  ...
  await dispatchGetPositiveCode16({ period: selectedPeriod.value, class1: "正1-6", class2: class2.value });
  loading.close();  
}
