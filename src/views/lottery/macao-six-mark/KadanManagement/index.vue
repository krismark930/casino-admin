<template>
  <el-card shadow="never" v-if="mainTableShow">
    <el-row :gutter="10">
      ...
      <el-col :span="7">
        <div class="flex">
          ...
          <el-button type="warning" @click="showKadanAddTable">添加会员</el-button>
        </div>
      </el-col>
    </el-row>
    <el-table
    ...
    >
      ...
      <el-table-column prop="memnum2" label="会" />
      <el-table-column prop="sf" label="代%" />
      <el-table-column prop="sj" label="总%" />
      <el-table-column prop="mjmj" label="股%" />
      <el-table-column label="公司%">
        <template #default="scope">
          <div>{{ scope.row.sf - scope.row.sj - scope.row.mjmj }}</div>
        </template>
      </el-table-column>
    ...
    <table width="99%" border="1" cellpadding="2" cellspacing="1" bordercolor="#ECE9D8">
      <tr>
        ...
        <td height="30" bordercolor="#CCCCCC">
          <el-select placeholder="" v-model="zong_id">
            <el-option
              v-for="(item, index) in kadanSuperiorList"
              :key="index"
              :label="item.label"
              :value="item.value"
            />
      ...
      <tr>
        <td height="30" colspan="4" bordercolor="#CCCCCC">
          ...
          >
            ...
            <tr v-for="(item, i) in kadanOtherList" :key="i">
      ...
      <tr>
        ...
        <td colspan="3" bordercolor="#CCCCCC">
          ...
          <el-button type="primary" @click="addKadan">保存会员</el-button>
...
import { memberStore } from "@/pinia/modules/macao_six_mark/member.js";
...
const { dispatchKadanSuperior } = memberStore();
const { dispatchAddKadan } = memberStore();
const mainTableShow = ref(true);
const addKadanTableShow = ref(false);
const page = ref(1);
...
const showKadanAddTable = async () => {
  mainTableShow.value = false;
  addKadanTableShow.value = true;
  await dispatchKadanSuperior();
};

const addKadan = async () => {
  if (kauser.value == "") {
    alert("用户名不能为空!");
    return;
  }
  ...
  await dispatchAddKadan(formData);
  if (!success.value) {
    ElNotification({
      title: "错误",
...
const kadanSuperiorList = computed(() => {
  const { getKadanSuperiorList } = storeToRefs(memberStore());
  return getKadanSuperiorList.value;
});

const kadanOtherList = computed(() => {
  const { getKadanOtherList } = storeToRefs(memberStore());
  return getKadanOtherList.value;
});
