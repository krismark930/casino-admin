...
export default {
  ...
  setup() {
    const { dispatchGetResultById } = bjpkResultStore();
    const { dispatchSaveLotteryResult } = bjpkResultStore();
    const { dispatchResultItemInitial } = bjpkResultStore();
    return {
  ...
  computed: {
    lotteryResultItem: function () {
      const { getLotteryResultData } = bjpkResultStore();
    ...
    errMessage: function () {
      const { getErrMessage } = bjpkResultStore();
