<template>
  ...
  >
    ...
    >
      ...
      <el-table-column property="top_middle_last" label="三连" align="center" />
      <el-table-column property="last" label="跨度" align="center" />
...
export default {
  ...
  setup() {
    const { dispatchB3ResultList } = b3ResultStore();
    const { dispatchCheckoutB3Result } = b3ResultStore();
    return {
      dispatchB3ResultList,
      dispatchCheckoutB3Result,
    };
  },
  data() {
    return {
  ...
  methods: {
    ...
    getB5ResultByFilter: async function () {
      ...
      this.loading = true;
      await this.dispatchB3ResultList(this.formData);
      this.loading = false;
    },
    reSettlement: async function (lottery_type, qishu) {
      ...
      this.loading = true;
      await this.dispatchCheckoutB3Result(formData);
      await this.dispatchB3ResultList(this.formData);
      this.successResult();
      this.loading = false;
    },
  ...
  computed: {
    b5ResultList: function () {
      const { getB3ResultList } = b3ResultStore();
      return getB3ResultList;
    },
    success: function () {
