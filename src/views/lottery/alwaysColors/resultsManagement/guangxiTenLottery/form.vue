...
export default {
  ...
  setup() {
    const { dispatchGetResultById } = gxsfResultStore();
    const { dispatchSaveLotteryResult } = gxsfResultStore();
    const { dispatchResultItemInitial } = gxsfResultStore();
    return {
  ...
  computed: {
    lotteryResultItem: function () {
      const { getLotteryResultData } = gxsfResultStore();
    ...
    errMessage: function () {
      const { getErrMessage } = gxsfResultStore();
