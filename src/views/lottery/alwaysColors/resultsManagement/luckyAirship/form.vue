...
export default {
  ...
  setup() {
    const { dispatchGetResultById } = xyftResultStore();
    const { dispatchSaveLotteryResult } = xyftResultStore();
    const { dispatchResultItemInitial } = xyftResultStore();
    return {
  ...
  computed: {
    lotteryResultItem: function () {
      const { getLotteryResultData } = xyftResultStore();
    ...
    errMessage: function () {
      const { getErrMessage } = xyftResultStore();
