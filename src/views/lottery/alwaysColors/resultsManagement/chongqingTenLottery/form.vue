...
export default {
  ...
  setup() {
    const { dispatchGetResultById } = cqsfResultStore();
    const { dispatchSaveLotteryResult } = cqsfResultStore();
    const { dispatchResultItemInitial } = cqsfResultStore();
    return {
  ...
  computed: {
    lotteryResultItem: function () {
      const { getLotteryResultData } = cqsfResultStore();
    ...
    errMessage: function () {
      const { getErrMessage } = cqsfResultStore();
