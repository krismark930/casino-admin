...
export default {
  ...
  setup() {
    const { dispatchGetResultById } = b5ResultStore();
    const { dispatchSaveLotteryResult } = b5ResultStore();
    const { dispatchResultItemInitial } = b5ResultStore();
    return {
  ...
  computed: {
    lotteryResultItem: function () {
      const { getLotteryResultData } = b5ResultStore();
    ...
    errMessage: function () {
      const { getErrMessage } = b5ResultStore();
