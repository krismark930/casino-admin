...
export default {
  ...
  setup() {
    const { dispatchGetResultById } = gd11ResultStore();
    const { dispatchSaveLotteryResult } = gd11ResultStore();
    const { dispatchResultItemInitial } = gd11ResultStore();
    return {
  ...
  computed: {
    lotteryResultItem: function () {
      const { getLotteryResultData } = gd11ResultStore();
    ...
    errMessage: function () {
      const { getErrMessage } = gd11ResultStore();
