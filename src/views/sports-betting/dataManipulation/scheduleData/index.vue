    ...
    <el-form :inline="true" :model="formData">
      <el-form-item label="类别">
        <el-select v-model="formData.gtype" @change="reload">
          <el-option v-for="item in gtypeOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="日期">
        <el-date-picker v-model="formData.dateStart" placeholder="" value-format="YYYY-MM-DD" style="width: 150px;"
          @change="reload"></el-date-picker>
      </el-form-item>
      <el-form-item label="选择联盟">
        <el-select v-model="formData.alliance" @change="getItems">
          <el-option v-for="item in allianceOptions" :key="item.value" :label="item.label"
            :value="item.value"></el-option>
        </el-select>
      </el-form-item>
    </el-form>
    <el-table :data="scheduleData" style="width: 100%;" border header-align="center" stripe v-loading="loading">
      ...
      <el-table-column property="sessions" label="场次" width="100px" align="center" />
      <el-table-column property="team" label="队伍" align="center" width="210px">
        <template #default="scope">
      ...
      <el-table-column property="fullFieldGoal" label="全场大小球" align="right" width="120">
        <template #default="scope">
      ...
      <el-table-column property="halfTimeHandicap" label="半场让球" align="center">
        <template #default="scope">
      ...
      <el-table-column property="halfCourtSize" label="半场大小" align="right" width="130">
        <template #default="scope">
      ...
      <el-table-column fixed="right" label="操作" width="150" align="center">
        <template #default="scope">
          <el-button-group>
            <el-button type="primary" @click="changeOdds(scope.$index, scope.row)" link>
              变更赔率
            </el-button>
            <el-button type="danger" @click="closeBet(scope.$index, scope.row)" link>
              关闭投注
            </el-button>
          </el-button-group>
          <el-button-group>
            <el-button type="primary" @click="relatedList(scope.$index, scope.row)" link>
              相关注单
            </el-button>
            <el-button type="danger" @click="deleteEvent(scope.$index, scope.row)" link>
              删除赛事
            </el-button>
          </el-button-group>
        </template>
