  ...
  <div style="border: 1px solid #eee; padding: 0.75rem; margin-top: 0.75rem; text-align: left; margin: 1rem;">
    ...
    <el-form :model="formData" inline="true">
      ...
      <el-form-item label="游戏名称:">
        <el-input v-model="formData.gameName" placeholder="" clearable></el-input>
      </el-form-item>
      <el-form-item label="">
        <el-button type="primary" @click="getGameResultByFilter">确认</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="gameList" v-loading="loading" style="width: 100%;" border header-align="center" stripe>
      ...
      <el-table-column property="GameType_H5" label="游戏代码(手机)" align="center" />
      ...
      <el-table-column property="Open" label="状态" align="center">
        <template #default="scope">
          <el-checkbox v-model="scope.row.Open" size="large" @change="updateAction(scope.row.ID, scope.row.Open)" />
        </template>
      </el-table-column>
      <el-table-column fixed="right" label="操作" width="100" align="center">
        <template #default="scope">
          <el-button-group>
            <el-button type="primary" link @click="modifyGame(scope.row)">
              修改
            </el-button>
            <el-button type="danger" link @click="deleteGame(scope.row.ID)">
              删除
            </el-button>
          </el-button-group>
        </template>
    ...
    <div class="pagination">
      <el-pagination background layout="prev, pager, next" :total="totalCount" :page-size="20"
        @current-change="onPageChange" v-model:current-page="formData.page_no" />
    </div>
    <el-dialog v-model="editDialogVisible" :title="dialogTitle" width="50%" center>
      <el-form :model="gameItem" label-width="150px" ref="formRef">
        <el-form-item label="游戏名称" prop="GameName" :rules="{
          required: true,
          message: '重要数据不能为空',
          trigger: 'blur',
        }">
        ...
        <el-form-item label="游戏名称(英文)">
          <el-input v-model="gameItem.GameName_EN" />
        </el-form-item>
        <el-form-item label="平台" prop="PlatformType" :rules="{
          required: true,
          message: '重要数据不能为空',
          trigger: 'blur',
        }">
          <el-select v-model="gameItem.PlatformType">
            <el-option v-for="item in addPlatformOptions" :key="item.value" :label="item.label"
              :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="游戏分类">
          <el-input v-model="gameItem.GameClass"></el-input>
        </el-form-item>
        <el-form-item label="游戏代码" prop="GameType" :rules="{
          required: true,
          message: '重要数据不能为空',
          trigger: 'blur',
        }">
          <el-input v-model="gameItem.GameType"></el-input>
        </el-form-item>
        <el-form-item label="游戏代码(H5)">
          <el-input v-model="gameItem.GameType_H5"></el-input>
        </el-form-item>
        <el-form-item label="图片" prop="ZH_Logo_File" :rules="{
          required: true,
          message: '重要数据不能为空',
          trigger: 'blur',
        }">
          <el-input v-model="gameItem.ZH_Logo_File"></el-input>
        </el-form-item>
        <el-form-item label="图片(H5)">
          <el-input v-model="gameItem.H5_Logo_File"></el-input>
        </el-form-item>
        <el-form-item label="上架日期">
          <el-date-picker v-model="gameItem.Date" placeholder="" value-format="YYYY-MM-DD"></el-date-picker>
        </el-form-item>
      </el-form>
      <template #footer>
